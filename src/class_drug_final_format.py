...
if len(sys.argv) == 4:
    ...
    header = 'SEX,AGEGRP,REGION,CATAGORY,REFILL_COUNT,ADMTYP,'
    f_butrans.write(header + ','.join(codes_butrans) + '\n')
    f_opana.write(header + ','.join(codes_opana) + '\n')
    for record in butrans:
        record = record.split(',')
        output = ','.join(record[1:6]) + ',' + record[-3]
        for code in codes_butrans:
            if code in record[6:-3]:
                output += ',1'
            else:
                output += ',0'
        f_butrans.write(output + '\n')
    for record in opana:
        record = record.split(',')
        output = ','.join(record[1:6]) + ',' + record[-3]
        for code in codes_opana:
            if code in record[6:-3]:
                ...
                output += ',0'
        f_opana.write(output + '\n')
    f_butrans.close()
    f_opana.close()
    print("Operation Successful :)")
else:
    print("Argument(s) missing.\n\
Please run the code as python3 class_drug_final_format.py DATASET.csv RULES.csv")
    sys.exit(1)
